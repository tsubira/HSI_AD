...
def scatter_hypercube_from_hyperspectral_image(hs_image, save_img = False, img_name = 'hs_image.jpg', size = 1):
    ...
    for layer in range(num_bands):
        ...
        ax.scatter(X, Y, num_bands-layer, c = band, cmap='jet', s = size)
    plt.axis('off')
    plt.show()
    if save_img:
